...
```{r}
ies_dat_probe %>% ezPlot(dv=ies, wid=participant, x= resp_rep, split = shape_rep, within=.(resp_rep, shape_rep)) + theme_classic() +
...
```
```{r}
ies_dat_probe %>% ezPlot(dv=ies, wid=participant, x= resp_rep, split = pos_fixed, within=.(resp_rep, pos_fixed)) + theme_classic() +
  theme(legend.position = 'bottom') + 
